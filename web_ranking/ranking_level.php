...
$file = $_SERVER['DOCUMENT_ROOT'] . '/cache/ranking_level.json';
$time = 3600;
...
$query = "SELECT TOP(20)
                ...
                _Guild.Name
            FROM
                SILKROAD_R_SHARD.._Char WITH (NOLOCK)
                INNER JOIN SILKROAD_R_SHARD.._Guild WITH (NOLOCK) ON _Char.GuildID = _Guild.ID
                                    
            WHERE
                _Char.deleted = 0 AND 
                _Char.CharID > 0 AND
                _Char.CharName16 NOT IN ('[GM]Eva', '[GM]Eve', '[GM]Huracan', '[GM]GD', '[GM]m1xawy')
            
            ORDER BY
                _Char.CurLevel DESC,
                _Char.ExpOffset DESC";
...
<head>
    ...
    <title>Silkroad Online - Level Ranking</title>
    ...
    <table class="table_rank" cellpadding="0" cellspacing="0">
        ...
        <?php foreach ($data as $value): ?>
                <tr onMouseOver='this.style.background="#2e261e"' onMouseOut='this.style.background="none"'>
                    ...
                    <td class="td5"><?= $value['CurLevel'] ?></td>
